...
def part1(input: str) -> int:
    ...
    computers = [Computer(instructions, [i]) for i in range(50)]

    while True:
        for i, computer in enumerate(computers):
            if computer.tick() is None:
                continue

            if len(computer.output_codes) == 3:
                address, x, y = computer.output_codes

                if address == 255:
                    return y

                computers[address].input_codes.append(x)
                computers[address].input_codes.append(y)
                computer.output_codes = []
    ...
    while True:
        for i, computer in enumerate(computers):
            ...
            if len(computer.output_codes) == 3:
                ...
                if all(len(c.input_codes) == 0 and len(c.output_codes) == 0 for c in computers):
                    computers[0].input_codes.append(X)
                    computers[0].input_codes.append(Y)
                    if Y in sent:
                        return Y
                    sent.add(Y)
